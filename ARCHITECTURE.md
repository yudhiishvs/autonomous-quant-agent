...
| Generic signed platform path | `PARTIALLY_IMPLEMENTED` | Canonical/hash, universe/experiment/profile, static CLI, owner-private secret-file, service-scoped runtime settings, and additive platform storage foundations plus tests |
...
| `platform.storage.engine` | Constructs a bounded SQLAlchemy engine from validated service settings; owns connection pooling but no schema or business state | `create_platform_engine()` | Runtime settings, hardened secret references, SQLAlchemy, psycopg, and SQLite | PostgreSQL URLs are loaded only from `AQA_DATABASE_URL_FILE`, query routing overrides fail, non-loopback hosts require full TLS verification, errors/parameters are redacted, and the offline path is fixed by settings; construction has no provider, broker, or order authority |
| `platform.storage.tables`, `migrations/` | Defines the 25 additive `aqa_*` tables, UTC type boundary, relationships, checks, indexes, append/state metadata, and a linear Alembic revision | SQLAlchemy metadata and `alembic upgrade head` | Platform domain values, SQLAlchemy, Alembic, PostgreSQL 16, and SQLite schema translation in isolated tests/offline mode | Runtime code never creates schema; migration preserves the collector schema, destructive downgrade raises before mutation, and PostgreSQL behavior remains externally validated only when the guarded CI service runs |
