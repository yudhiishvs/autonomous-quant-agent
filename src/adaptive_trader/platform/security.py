...
def _open_local_secret_directory(
    root_descriptor: int,
    *,
    directory_flags: int,
    create: bool = True,
...
def load_local_bootstrap_secret(application_root: Path, name: str) -> RedactedSecret:
    """Load one fixed local infrastructure secret through the bootstrap boundary.

    This is deliberately separate from the runtime service-file namespace. It accepts only a
    filename generated by :func:`bootstrap_local_secrets`, requires the same owner-private root,
    directory, and file invariants, and never renders the filename or value in an error.
    """

    if type(name) is not str or name not in LOCAL_BOOTSTRAP_FILENAMES:
        raise _bootstrap_error("secret name is not supported")

    with _LOCAL_BOOTSTRAP_LOCK:
        directory_flags, _file_flags, existing_file_flags = _bootstrap_open_flags()
        root_descriptor = -1
        secret_directory_descriptor = -1
        failure: LocalSecretBootstrapError | None = None
        loaded: RedactedSecret | None = None
        try:
            root_descriptor = _open_bootstrap_root(
                application_root,
                directory_flags=directory_flags,
            ...
            _reject_stale_bootstrap_temporaries(secret_directory_descriptor)
            loaded = _validate_existing_local_secret(
                name,
                directory_descriptor=secret_directory_descriptor,
                existing_file_flags=existing_file_flags,
            )
            ...
            failure = _bootstrap_error("secret could not be loaded safely")
        finally:
            for descriptor, resource in (
                (secret_directory_descriptor, "the secret directory"),
                (root_descriptor, "the application root"),
        ...
        if failure is not None:
            raise failure
        if loaded is None:
            raise _bootstrap_error("secret could not be loaded safely")
        return loaded
