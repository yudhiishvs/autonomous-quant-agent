...
"Working" has three separate meanings:

1. **Software correctness:** the collector, features, backtester, risk engine, and executor produce known, deterministic results.
...
Start with tiny fixtures whose correct result is calculated independently of the backtester. For example, under `iex_bar_cost_v1`, a 10-share long fixture with entry VWAP `$100`, exit VWAP `$102`, and 18 adverse basis points on each side must produce:

~~~text
entry fill = 100 × (1 + 18 / 10,000) = 100.18
exit fill  = 102 × (1 - 18 / 10,000) = 101.8164
net P&L    = 10 × (101.8164 - 100.18) = 16.364 → $16.36
~~~

Equivalent fixtures cover a profitable short, a losing long, partial target changes, a direct long-to-short reversal, the 3:44–3:45 p.m. forced exit, base/1.5×/2× costs, session-loss flattening, and fold termination. Expected values are stored as test data rather than generated by the production implementation.

The target pytest suite includes at least:

~~~text
test_canonical_bar_rejects_invalid_ohlcv
test_gap_repair_is_idempotent
test_historical_realtime_aggregation_parity
test_features_never_use_future_bars
test_unregistered_or_forward_feature_is_rejected
test_signal_waits_for_watermark_and_execution_window
test_long_and_short_pnl_match_known_answers
test_costs_reduce_pnl_and_scale_under_stress
test_position_episode_and_reversal_counts
test_positions_flatten_before_close
test_session_and_fold_risk_limits
test_duplicate_order_intent_is_idempotent
test_replay_is_deterministic
test_holdout_can_be_opened_only_once
test_unapproved_symbol_is_rejected
test_live_money_hostname_is_rejected
test_missing_or_revoked_credential_disables_execution
~~~
...
- enforce the canonical schema, UTC timestamps, provider/feed/adjustment identity, and symbol roles;
- reject impossible OHLC values, negative volume, invalid intervals, and missing execution VWAP;
- detect duplicates, corrections, out-of-order events, stale bars, and calendar-aware gaps;
- prove that reconnect/backfill is idempotent and does not silently lose or duplicate bars;
...
### Backtester and model sanity controls

Before any candidate may approach the holdout:

- run the known-answer ledger through both the event-driven backtester and an independent simple calculation;
- run the same frozen dataset, image, configuration, and seed twice and require identical predictions, ledgers, decisions, reports, and artifact hashes;
- inject a forbidden forward-return feature into a synthetic fixture and require schema/leakage validation to reject it;
- train a diagnostic model on shuffled labels and another on registered random-noise features;
- report prediction/class proportions, calibration, turnover, exposure, and per-symbol/fold results;
- compare XGBoost with logistic regression, momentum, mean reversion, equal weight, and always-FLAT under identical timing, risk, and costs.

The shuffled-label and noise controls are permanently ineligible for promotion and never receive holdout access. They should not systematically outperform the baselines or pass the development gate; if either does, all research is blocked until leakage, selection bias, or a backtester defect is ruled out.

### End-to-end replay and failure injection

One frozen historical day must replay through:

~~~text
one-minute events
→ 15-minute aggregation and watermark
→ feature vector
→ model probabilities and target
→ risk decision
→ simulated order intent/fill
→ reconciliation, receipt, and report
~~~

Two clean runs must produce the same correlation IDs, decisions, ledger, and hashes. Then repeat while deliberately interrupting the collector, database, research worker, inference worker, and executor at critical boundaries: after event receipt but before storage, after order intent but before submission, after submission but before response persistence, and before reconciliation. Also inject stale/missing data, duplicate messages, timeouts, database unavailability, rejected/partial fills, and a revoked credential.

The required outcome is recovery without lost bars or duplicate decisions, or a fail-closed halt with an auditable error. An ambiguous order state must never trigger a blind resubmission.

### Shadow verification

Only the exact holdout-passed artifact may enter shadow mode. During at least 10 completed trading days and 50 eligible signals:

- submit no broker order;
- compare each recorded live feature vector with an offline recomputation from the frozen raw events;
- require exact feature equality and prediction probabilities within `1e-9` when using the same recorded image and model artifact;
- verify watermark, latency, availability-mask, stale-data, and forced-exit behavior;
- restart each shadow workload and prove that no decision is missed or duplicated;
- require zero unresolved data, model, risk, deployment, or reconciliation failures.

Any mismatch returns the model to a non-active state. Shadow observations cannot be used to retune the already consumed holdout generation.

### Alpaca paper verification

Paper submission requires explicit operator authorization and a shadow-passed model. Start with the minimum practical simulated position and verify the complete lifecycle:

1. approved signal and independent risk decision;
2. durable order intent and unique client order ID;
3. paper-host-only submission;
4. accepted, rejected, canceled, partial-fill, and filled updates where the provider permits them, with fakes covering cases that cannot be forced safely;
5. local-versus-Alpaca position/order reconciliation;
6. kill-switch flatten/disable behavior;
7. disconnect, timeout, and credential-revocation behavior;
8. tamper-evident receipt with no secret values.

If every research candidate is rejected, no real Alpaca paper order is submitted merely to complete the demo. The executor remains verified with fakes, replay, observer mode, and dry-run paper-endpoint tests, while the UI demonstrates that activation is blocked.

### Stage gates and required evidence

| Stage | Required pass condition | Failure action |
| --- | --- | --- |
| Unit and contract tests | Every known-answer, symbol-boundary, timing, cost, risk, and endpoint-safety test passes | Block merge |
| Data integration | Schema, parity, correction, restart, and gap-repair checks pass | Block dataset freeze |
| Development evaluation | Every predeclared development/promotion rule passes | Bounded registered revision or `REJECTED` |
| Locked holdout | Every numerical holdout rule passes once | Permanently `REJECTED`; no revision |
| Shadow | At least 10 days, 50 eligible signals, parity within tolerance, and zero unresolved failures | Deactivate and investigate |
| Alpaca paper | Explicit authorization, paper endpoint, idempotency, reconciliation, and kill switch all verified | Disable submission |

Preserve JUnit/pytest results, coverage reports, data-quality reports, known-answer expected-versus-actual ledgers, negative-control reports, replay hashes, walk-forward/holdout evidence, shadow parity reports, and conditional paper receipts. Each artifact records the dataset, code, dependency/image, model, evaluator, cost-policy, and risk-policy versions needed to reproduce it.
