...
## Active user directive — 2026-09-06

`Completion` refers to the **Complete Non-AI Architecture Implementation Directive —
Code / Tests / Configuration / Infrastructure-as-Code Only; Main AI System Strictly Frozen**.
It supersedes the earlier live-data-first Phase Zero gate. `Core` and `Harness` references
below remain historical sources for nonconflicting contracts and normative formulas. The
inherited dirty worktree is preserved; historical verification never certifies current code.

This assignment completes repository implementation across collection, PostgreSQL persistence,
datasets, scheduling, risk/execution infrastructure, private API/dashboard, worker lifecycles,
observability, runtime configuration, security, CI, packaging, and engineering harnesses.
Do not deploy, obtain credentials, provision paid or cloud resources, call Alpaca during ordinary
verification, or require real provider/hosted database activation before completing code.
External adapters may be `IMPLEMENTED_NOT_EXTERNALLY_VALIDATED` when their complete contracts
are tested. Missing required non-AI code cannot be relabeled an external limitation.

The protected main AI remains `OUT_OF_SCOPE_FROZEN_AI`: no model, feature/label intelligence,
research strategy, training, promotion logic, or protected configuration changes are authorized.
The [61-file manifest](evidence/main-ai-freeze.json) and `scripts/verify_main_ai_freeze.py`
verify the existing boundary; its latest check matched all 61 hashes. This scope classification
is distinct from the six implementation-status labels. Existing AI requirement IDs and formulas
below are retained for provenance and carry this scope override wherever they require AI edits.

| Conflict | Active resolution |
| --- | --- |
| Previous live-data-first gate and real authentication acceptance | Superseded by Completion §§0–4: implement every non-AI slice with offline/contract verification. |
| Previous deployment, always-on host, and hosted recovery requirements | Supply code/configuration/runbooks; external activation is intentionally not performed. |
| Collection versus research authority | Retain the 29-symbol intake and frozen 11-symbol research/benchmark/context scope; extra collection membership grants no execution authority. |
| Credential namespaces | Canonical services consume dedicated file-backed `AQA_*` settings; legacy compatibility must reject ambiguous simultaneous sources. |
| Main-AI requirements in historical ledger | `OUT_OF_SCOPE_FROZEN_AI`; preserve approved behavior and protected hashes, with no tuning or implementation changes. |
| Historical unimplemented/verified rows | Historical evidence only where superseded by current source; this overlay and the active plan govern current acceptance pending final verification. |

### Completion traceability

The [active execution plan](execution-plans/platform-core.md) owns exact command evidence and
completion. Latest combined offline baseline: 2,015 passed and one failure; the failure received
a non-AI correction with 49 focused tests passing. Dataset causality review subsequently passed
62 focused tests. A fresh PostgreSQL attempt was refused by the missing disposable-cluster guard;
no new passing PostgreSQL result is claimed here. Current reversal/paper-runtime review and the
final combined harness remain pending. These are verification states, not external activation gates.

| ID | Requirement | Source | Implementation / evidence | Status |
| --- | --- | --- | --- | --- |
| REQ-USER-001 | Preserve protected main-AI paths and inherited state. | Completion §2 | 61-file manifest and verified hashes; scope `OUT_OF_SCOPE_FROZEN_AI` | IMPLEMENTED_AND_VERIFIED |
| REQ-USER-002 | Complete all non-AI code with truthful local/contract evidence; no live activation prerequisite. | Completion §§0–4,42 | Complete source/harness review and final verification pending | PARTIALLY_IMPLEMENTED |
| REQ-USER-003 | Forbid real-money capability; isolate data and paper credentials; keep submission disabled by default. | Completion §§7,36 | Service settings, image boundaries, execution gates and safety tests; final review pending | PARTIALLY_IMPLEMENTED |
| REQ-COMPLETE-001 | Provide persistent scheduling, independent risk, intent-first execution, restart/reconciliation, private control API, dashboard and bounded workers. | Completion §§6,8,11–18,36 | `platform/scheduling`, `risk`, `execution`, `api`, `control`, `dashboard`, worker runtime and focused tests; final acceptance pending | PARTIALLY_IMPLEMENTED |
| REQ-COMPLETE-002 | Supply reproducible full-check, security/CI/package/container/backup/benchmark harness and honest documentation. | Completion §§19–43 | Makefile, scripts, workflows and runbooks; final combined execution pending | PARTIALLY_IMPLEMENTED |
| REQ-SCHEDULED-001 | Run one bounded collection invocation from PostgreSQL checkpoints with overlap, gaps, derived state, fenced ownership and durable-before-checkpoint writes. | Completion §§9–10.1 | `aqa data collect-once`, `collection/service.py`, canonical/recovery/derived modules, entrypoint/service tests | PARTIALLY_IMPLEMENTED |
| REQ-SCHEDULED-002 | Provide opt-in GitHub catch-up scheduling, explicit dispatch activation, concurrency control and bounded timeouts; delayed/missed invocations recover from PostgreSQL. | Completion §§10.2–10.3 | `.github/workflows/market-data.yml`; `tests/safety/test_scheduled_collection_workflow.py`; no remote invocation performed | PARTIALLY_IMPLEMENTED |
| REQ-SCHEDULED-003 | Use only dedicated file-backed data credentials, publish no secrets/raw data, and document honest one-shot health and manual activation. | Completion §10 | Collection runtime validation, workflow secret-file cleanup, `docs/scheduled_market_data.md`, credentials and operations tests | PARTIALLY_IMPLEMENTED |

### Retained data requirement identifiers

The earlier `REQ-PZ-*` identifiers remain stable references, but no longer establish a live
activation phase or sequencing gate. Their scope is repository code and deterministic/contract
verification under Completion §9. Final integrated evidence is pending; prior local results remain
historical records in [market-data evidence](evidence/market-data-activation.md).

| ID | Retained requirement | Active source | Current evidence boundary | Status |
| --- | --- | --- | --- | --- |
| REQ-PZ-001 | Collect exactly the versioned 29 symbols with 8 active, 1 benchmark, 2 context, and 18 collection-only roles. | Completion §§2–3,9; inherited data contract retained | Inspected collector/data code and focused tests; final combined verification pending | PARTIALLY_IMPLEMENTED |
| REQ-PZ-002 | Have one canonical production ingestion path, shared normalization, explicit raw/canonical owners, and deliberate compatibility. | Completion §§2–3,9; inherited data contract retained | Inspected collector/data code and focused tests; final combined verification pending | PARTIALLY_IMPLEMENTED |
| REQ-PZ-003 | Use fixed Alpaca data endpoints, IEX/raw/1Min provenance, explicit timeouts, and observable bounded retry. | Completion §§2–3,9; inherited data contract retained | Inspected collector/data code and focused tests; final combined verification pending | PARTIALLY_IMPLEMENTED |
| REQ-PZ-004 | Authenticate and paginate historical data safely, preserve provenance, bound requests, resume checkpoints, and reconcile downtime without invented bars. | Completion §§2–3,9; inherited data contract retained | Inspected collector/data code and focused tests; final combined verification pending | PARTIALLY_IMPLEMENTED |
| REQ-PZ-005 | Authenticate and receive acknowledgements for all 29 WebSocket subscriptions; handle malformed/duplicate/late/updated data, failures, lease loss, and shutdown. | Completion §§2–3,9; inherited data contract retained | Inspected collector/data code and focused tests; final combined verification pending | PARTIALLY_IMPLEMENTED |
| REQ-PZ-006 | Preserve append-only observations and explicit canonical corrections; duplicates must not create economic revisions or double count. | Completion §§2–3,9; inherited data contract retained | Inspected collector/data code and focused tests; final combined verification pending | PARTIALLY_IMPLEMENTED |
| REQ-PZ-007 | Converge canonical and derived state under legal out-of-order delivery and interruption. | Completion §§2–3,9; inherited data contract retained | Inspected collector/data code and focused tests; final combined verification pending | PARTIALLY_IMPLEMENTED |
| REQ-PZ-008 | Persist a meaningful gap lifecycle, bound same-series repair, and block required research coverage without fabricating missing bars. | Completion §§2–3,9; inherited data contract retained | Inspected collector/data code and focused tests; final combined verification pending | PARTIALLY_IMPLEMENTED |
| REQ-PZ-009 | Distinguish live ownership, subscription, REST coverage, fresh canonical data, and research readiness across session boundaries. | Completion §§2–3,9; inherited data contract retained | Inspected collector/data code and focused tests; final combined verification pending | PARTIALLY_IMPLEMENTED |
| REQ-PZ-010 | Use deliberate compatible exchange-calendar semantics without phantom gaps on holidays, overnight, DST, or early closes. | Completion §§2–3,9; inherited data contract retained | Inspected collector/data code and focused tests; final combined verification pending | PARTIALLY_IMPLEMENTED |
| REQ-PZ-011 | Materialize exact complete 15-minute aggregates with deterministic lineage and revisions; compute contiguous symbol and active-basket readiness. | Completion §§2–3,9; inherited data contract retained | Inspected collector/data code and focused tests; final combined verification pending | PARTIALLY_IMPLEMENTED |
| REQ-PZ-012 | Enforce the corporate-action invalidation and listing/metadata boundary without silently approving unknown or changed inputs. | Completion §§2–3,9; inherited data contract retained | Inspected collector/data code and focused tests; final combined verification pending | PARTIALLY_IMPLEMENTED |
| REQ-PZ-013 | Publish immutable read-back-validated dataset bytes and manifests tied to exact universe, lineage, corrections, coverage, calendar, and policy. | Completion §§2–3,9; inherited data contract retained | Inspected collector/data code and focused tests; final combined verification pending | PARTIALLY_IMPLEMENTED |
| REQ-PZ-014 | Use persistent PostgreSQL, explicit migrations, constrained transactions, least privilege, bounded connections and verified remote TLS. | Completion §§2–3,9; inherited data contract retained | Inspected collector/data code and focused tests; final combined verification pending | PARTIALLY_IMPLEMENTED |
| REQ-PZ-015 | Advance checkpoints only after durable data; enforce one expiring fenced owner and recover from crash without losing history. | Completion §§2–3,9; inherited data contract retained | Inspected collector/data code and focused tests; final combined verification pending | PARTIALLY_IMPLEMENTED |
| REQ-PZ-016 | Reconcile reconnect overlap and older gaps, release/expire ownership safely, and recover the same canonical state after restart. | Completion §§2–3,9; inherited data contract retained | Inspected collector/data code and focused tests; final combined verification pending | PARTIALLY_IMPLEMENTED |
| REQ-PZ-017 | Converge secure configuration, reject ambiguous credentials, persist immutable universe/history identity, and expose no secrets. | Completion §§2–3,9; inherited data contract retained | Inspected collector/data code and focused tests; final combined verification pending | PARTIALLY_IMPLEMENTED |
| REQ-PZ-018 | Provide data-only Compose supervision and persistent-database configuration for future operator activation. | Completion §§2–3,9; inherited data contract retained | Inspected collector/data code and focused tests; final combined verification pending | PARTIALLY_IMPLEMENTED |
| REQ-PZ-019 | Implement and contract-test REST and stream authentication, entitlement failures, subscriptions, catch-up, and persisted readback; actual provider activation is not required. | Completion §§2–3,9; inherited data contract retained | Inspected collector/data code and focused tests; final combined verification pending | PARTIALLY_IMPLEMENTED |
| REQ-PZ-020 | Implement and test restart, transient recovery, singleton ownership, and freshness monitoring through deterministic local contracts. | Completion §§2–3,9; inherited data contract retained | Inspected collector/data code and focused tests; final combined verification pending | PARTIALLY_IMPLEMENTED |
| REQ-PZ-021 | Provide guarded logical backup/restore implementation and disposable-database verification. | Completion §§2–3,9; inherited data contract retained | Inspected collector/data code and focused tests; final combined verification pending | PARTIALLY_IMPLEMENTED |
| REQ-PZ-022 | Record truthful implementation/review evidence without secrets; close confirmed HIGH/BLOCKER findings before completion. | Completion §§2–3,9; inherited data contract retained | Inspected collector/data code and focused tests; final combined verification pending | PARTIALLY_IMPLEMENTED |
| REQ-PZ-023 | Preserve baseline failures as evidence, repair non-AI root causes, and verify without changing protected AI. | Completion §§2–3,9; inherited data contract retained | Inspected collector/data code and focused tests; final combined verification pending | PARTIALLY_IMPLEMENTED |
...
| ID | Requirement | Source | Implementation location | Verification | Status | Assumption |
| --- | --- | --- | --- | --- | --- | --- |
| REQ-FUNC-001 | Deliver a self-hosted, single-operator, open-source platform supporting offline simulation, shadow operation, and paper execution only; real-money trading is unsupported. | Core §§0,2 | `src/adaptive_trader/platform/`; profiles | Mode/route/import tests and offline end-to-end test | IMPLEMENTED_AND_VERIFIED | All profiles and code paths are closed to real-money execution; external paper behavior remains separately unvalidated. |
| REQ-FUNC-002 | Default installation requires no broker credentials, makes no external network calls, uses deterministic fixture data and a fake broker, and has submission disabled. | Core §§2,11.3,21,33,40 | `configs/platform/offline.yaml`; platform demo | Socket-denied demo and startup tests | IMPLEMENTED_AND_VERIFIED | The twice-run demo uses isolated SQLite and the fake broker without loading or constructing Alpaca boundaries. |
| REQ-FUNC-003 | Shadow mode persists validated decisions and dry-run plans without constructing or calling a broker. | Core §§2,11.3 | `configs/platform/shadow.yaml`; strategy worker | Shadow integration and import-boundary tests | PARTIALLY_IMPLEMENTED | `shadow propose-once` persists registered AlwaysFlat proposals under strategy authority; `shadow run-once` consumes durable signals under execution authority and persists validated risk and dry-run plans without a broker client. Focused tests pass; final combined PostgreSQL and harness evidence remains pending. |
| REQ-FUNC-004 | Paper support is disabled in tracked configuration and requires all independent authorization gates; the default verifier denies approval. | Core §§2,18,20.6 | paper profile; authorization verifier; paper adapter | Every-gate negative tests and prohibited-endpoint scan | IMPLEMENTED_NOT_EXTERNALLY_VALIDATED | All gates/default denial/literal-paper constraints are implemented; no paper connection or order occurred. |
| REQ-FUNC-005 | Strategies are untrusted producers of declarative, versioned, signed proposals; the platform alone validates, risks, plans, persists, submits, and reconciles. | Core §§0,2,18–20 | `platform/signals`, `platform/risk`, `platform/execution` | Import-boundary, proposal-tamper, and risk-bypass tests | IMPLEMENTED_AND_VERIFIED | Strict envelopes, import tests, signed risk, and intent-first execution enforce the separation. |
| REQ-FUNC-006 | Consequential transitions are auditable and ambiguous state fails closed. | Core §§2,20,22,27 | repositories, audit chain, latches | Failure injection, tamper, ambiguity, and restart tests | IMPLEMENTED_AND_VERIFIED | Tamper, timeout-after-acceptance, restart, reconciliation, latch, and replay tests prove fail-closed behavior. |
| REQ-FUNC-007 | A new user can install, validate config, start the offline stack, run a deterministic demo, inspect status, and implement a registered signal provider without broker-recovery knowledge. | Core §§2,32–33 | package metadata, CLI, Compose, README, `examples/` | Clean-environment quickstart and extension validation | IMPLEMENTED_NOT_EXTERNALLY_VALIDATED | CLI/demo/example/quickstart exist and are locally exercised; clean package/container installation evidence is reported separately. |
| REQ-FUNC-008 | Preserve `adaptive_trader`, `adaptive-portfolio-agent`, legacy commands, tests, backtest, replay, and long-only contracts while adding `autonomous-quant-agent` and `aqa` aliases. | Core §§3,9,18,19,20,36 | `pyproject.toml`; composed CLI; compatibility adapters | Legacy suite/backtest/replay and alias smoke tests | IMPLEMENTED_AND_VERIFIED | Legacy and new entry points coexist; backtest/replay remain mandatory regression gates. |
...
| ID | Requirement | Source | Implementation location | Verification | Status | Assumption |
| --- | --- | --- | --- | --- | --- | --- |
| REQ-ARCH-001 | New generic code uses the exact cohesive `src/adaptive_trader/platform/` module tree for canonical, config, domain, data, storage, scheduling, signals, risk, execution, jobs, API, and observability behavior; no empty placeholders. | Core §9 | `src/adaptive_trader/platform/` | Tree/content inspection and import tests | IMPLEMENTED_AND_VERIFIED | All required responsibilities exist as nonempty platform modules; the private API is named `control` to make its authority explicit. |
| REQ-ARCH-002 | Dependency direction is market data → canonical state → signal → risk → intent → execution policy → broker adapter; external I/O remains in adapters and broker objects do not enter the domain core. | Core §5; Harness §36A | Platform packages and architecture tests | AST/import-boundary tests | IMPLEMENTED_AND_VERIFIED | Dedicated AST/import tests enforce data, signal, risk, execution, storage, and control boundaries. |
| REQ-ARCH-003 | Collector may access only its scoped database credential and, for live data, Alpaca data credentials plus validated experiment/data/artifact paths; it has no paper credential, trading client, order/risk-control write, arbitrary URL, shell, or plugin authority. | Core §§5.1–5.2,15 | `platform/data/`; collector container/role | Import, credential-mount, grant, URL, and subprocess tests | IMPLEMENTED_NOT_EXTERNALLY_VALIDATED | Code, role, and container tests enforce the scoped boundary; no live data credential or connection was exercised. |
| REQ-ARCH-004 | Scheduler may access only its scoped database credential and experiment/calendar/watermark/slot state; it has no Alpaca credential, broker, plugin-internal, order, or arbitrary-network authority. | Core §5.2 | `platform/scheduling/`; scheduler role/container | Import/grant/container tests | IMPLEMENTED_AND_VERIFIED | Import, runtime-setting, role, and container configuration tests enforce the closed authority set. |
| REQ-ARCH-005 | Strategy worker may access only its scoped database credential, immutable decision context, and registered providers; it writes signals/audit only and cannot access paper secrets, brokers, DDL, risk mutation, API-controlled arbitrary imports, or orders. | Core §§2.2,5.2 | `platform/signals/`; strategy role/container | Import/grant/config tests | IMPLEMENTED_AND_VERIFIED | Provider, import, runtime-setting, grant, and container tests enforce the boundary; installed Python remains operator-trusted. |
| REQ-ARCH-006 | Control API may access only its scoped database credential and operator token, with safe-read, bounded-job, and audited halt/resume authority; it cannot access Alpaca credentials, brokers, or any direct trade/arbitrary-code/path/URL/SQL operation. | Core §§5.2,24 | `platform/control/`; control role/container | Route inventory, import, auth, schema, and grant tests | IMPLEMENTED_AND_VERIFIED | The API and control role expose only safe reads and bounded jobs/latch controls with no order/fill DML. |
| REQ-ARCH-007 | Dashboard calls only private API read routes using its own API-token file and has no database, Alpaca credential, other filesystem-secret, broker, order control, or mutation client surface. | Core §§5.2,26 | `platform/dashboard/`; Compose | Import/config/client-route tests | IMPLEMENTED_AND_VERIFIED | The read-only token scope and route allowlist resolve dashboard authorization without database access. |
| REQ-ARCH-008 | Execution/reconciliation may access its scoped database credential and, only in the paper service, paper-broker credentials; it has no data-provider credential, model/plugin loading, arbitrary live endpoint, DDL, or real-money capability. | Core §5.2 | `platform/execution/`; execution role/container | Import, credential matrix, endpoint, and grant tests | IMPLEMENTED_NOT_EXTERNALLY_VALIDATED | Signed behavior, role, mount, import, and endpoint tests enforce the boundary; no paper credential or external call was used. |
| REQ-ARCH-009 | Automated boundary tests reject forbidden collector/API/dashboard/strategy/execution imports, dangerous deserializers or code execution, generic SDK credential variables, and configurable live endpoints. | Core §5.3; Harness §23 | `tests/architecture/`, `tests/safety/` | Isolated subprocess and AST/static tests | IMPLEMENTED_AND_VERIFIED | Exact allowlists and prohibited-authority scans cover each generic platform boundary. |
| REQ-ARCH-010 | Use Python 3.11+, `uv`/`uv.lock`, immutable dataclasses internally, strict Pydantic v2 externally, SQLAlchemy 2, Alembic, PostgreSQL 16, PyArrow, FastAPI, Typer, Streamlit-through-API, pytest, Ruff, mypy, Prometheus metrics, and structured JSON logs. | Core §3 | `pyproject.toml`; platform modules; containers | Locked install, package, type, service, and dependency tests | IMPLEMENTED_NOT_EXTERNALLY_VALIDATED | The complete stack and locked metadata are present; clean package/container/PostgreSQL validation is reported only from executed gates. |

## Configuration, experiment, time, numeric, and secret contracts

| ID | Requirement | Source | Implementation location | Verification | Status | Assumption |
| --- | --- | --- | --- | --- | --- | --- |
| REQ-CONFIG-001 | External/YAML/API models are frozen, strict, reject unknown fields, and produce immutable `PlatformConfig`, `RuntimeSettings`, universe, market-data, session, execution, and risk contracts. | Core §11 | `platform/config.py`, `platform/universe.py`, `platform/domain.py` | Positive/unknown-field/mutation/hash tests | IMPLEMENTED_AND_VERIFIED | Strict configuration and public domain/API contracts are immutable, bounded, and reject unknown fields. |
...
| ID | Requirement | Source | Implementation location | Verification | Status | Assumption |
| --- | --- | --- | --- | --- | --- | --- |
| REQ-DATA-001 | `CanonicalBar` contains provider/feed/adjustment/symbol/timeframe, UTC interval and receipt/provider timestamps, Decimal OHLCV-related fields, schema/source/payload identity, quality flags, and correction metadata; identity is provider/feed/adjustment/symbol/timeframe/start. | Core §12 | `platform/domain.py`, `platform/data/normalization.py` | Construction and canonical hash tests | IMPLEMENTED_AND_VERIFIED | Frozen canonical bars and persistent revisions validate and hash every required field with deterministic identity and lineage. |
| REQ-DATA-002 | Bar validation enforces allowlist, role, supported series, UTC aligned start-inclusive/end-exclusive interval, positive coherent OHLC, nonnegative volume/trades/VWAP, finiteness, schema, and payload hash; execution-reference minute bars require VWAP. | Core §12 | normalization/provider boundary | Positive, negative, and property tests | IMPLEMENTED_AND_VERIFIED | Normalization tests cover role/series/time alignment, coherent finite Decimal values, execution-reference VWAP, flags, schema, and payload identity. |
| REQ-DATA-003 | Equivalent historical REST and real-time WebSocket events use one normalizer and yield byte-equivalent canonical payloads; feed is exactly IEX and adjustment raw with no fallback. | Core §§12.1,15 | `platform/data/normalization.py`, provider adapter | Historical/stream parity fixtures | IMPLEMENTED_AND_VERIFIED | Historical and stream fixtures converge through one canonical boundary; external connectivity is tracked separately. |
| REQ-DATA-004 | Ingestion is append-only: first effective payload inserts revision 1, identical delivery is a duplicate, changed payload appends revision N+1, and latest projection updates atomically without overwriting history. | Core §12.2 | storage repositories/tables | Known-answer, rollback, concurrent duplicate/correction tests | IMPLEMENTED_NOT_EXTERNALLY_VALIDATED | The generic platform repository implements INSERTED/DUPLICATE/CORRECTED outcomes, immutable N+1 correction links, independently checked semantic/provenance hashes, and an atomic version-fenced latest projection across SQLite and PostgreSQL paths. Final current-revision PostgreSQL 16 and published-CI evidence is pending. |
| REQ-DATA-005 | Persist calendar-aware gaps before repair; repair is bounded, same-series, idempotent, and remains unresolved on partial/ambiguous results; post-decision corrections create incidents and never retroactive orders. | Core §12.3 | `platform/data/watermarks.py`, collector, repositories | Holiday/overnight/repair/correction tests | IMPLEMENTED_AND_VERIFIED | Gap lifecycle and exact repair coverage are durable and correction handling never mutates an earlier decision. |
| REQ-DATA-006 | Pure 1Min→15Min aggregation requires exactly 15 session-aligned minutes and calculates OHLC, summed volume, conditional trade count, volume-weighted VWAP, and ordered constituent lineage exactly; incomplete/cross-session buckets are ineligible. | Core §13 | `platform/data/aggregation.py` | Independent known-answer fixture | IMPLEMENTED_AND_VERIFIED | Independent known-answer tests cover complete, incomplete, and cross-session buckets. |
| REQ-DATA-007 | Aggregation converges under out-of-order arrivals, shares historical/stream logic, revisions on effective corrections, and derives DST/early-close behavior from the calendar; benchmark/context aggregates never block active readiness. | Core §13 | aggregation/materialization service | Permutation, correction, calendar, restart tests | IMPLEMENTED_AND_VERIFIED | Materialization persists deterministic revisions and converges after reordered arrivals/corrections. |
| REQ-DATA-008 | Track highest contiguous quality-approved watermark per series and the minimum across active symbols; gaps/failed transactions/missing active symbols block progress, context does not, and restart reconstructs auditable state. | Core §16 | `platform/data/watermarks.py`; repositories | Gap, rollback, restart, and basket tests | IMPLEMENTED_AND_VERIFIED | Symbol and active-basket watermarks reconstruct from durable eligible bars and fail closed across gaps or missing active data. |
| REQ-DATA-009 | A generic provider boundary has deterministic fixture and Alpaca historical/stream adapters; provider hosts are constants and there is no scraping, trading client, arbitrary URL, or live credential validation in this program. | Core §15 | `platform/data/provider.py`, `collector.py` | Fake-client contract, import, endpoint, and socket-denial tests | IMPLEMENTED_NOT_EXTERNALLY_VALIDATED | Fixture and fixed-host Alpaca data adapters are mock-tested; no credential or external connection was used. |
| REQ-DATA-010 | Collector subscribes only to experiment collection symbols, validates before write, audits connection state, uses 1/2/4/8/16-second bounded retries and explicit rate limits, persists before watermark, handles ordering/gaps, and resumes durably without logging credentials. | Core §15 | platform collector service/container | Retry-clock, rate-limit, restart, ordering, redaction tests | IMPLEMENTED_NOT_EXTERNALLY_VALIDATED | Deterministic adapter/service tests cover bounds, ordering, recovery, and redaction; no hosted/provider operation was performed. |
| REQ-STORE-001 | Operational persistence uses a redacted SQLAlchemy URL from `AQA_DATABASE_URL_FILE`; Compose uses PostgreSQL 16, while SQLite is restricted to isolated tests/offline legacy/demo; SQL is bound and transactions explicit. | Core §14.1 | `platform/storage/engine.py`; Compose | Dialect, redaction, SQL-injection, and PG16 integration tests | PARTIALLY_IMPLEMENTED | The platform engine now loads only the hardened database secret reference, redacts failures, rejects routing overrides, requires verified TLS off loopback, applies bounded connection settings, and maps validated offline paths to SQLite. The target Compose operational path remains. |
| REQ-STORE-002 | Alembic upgrade from empty state creates the platform schema; operational startup never uses uncontrolled `create_all`; destructive downgrade of core market/order/audit data fails clearly. | Core §14.2 | `alembic.ini`, `migrations/` | Empty DB, prior-state upgrade, downgrade refusal, drift tests | IMPLEMENTED_NOT_EXTERNALLY_VALIDATED | Additive revisions create the platform schema, preserve and validate legacy bar revisions while adding semantic/lineage hashes, then establish governed ownership/grants. Destructive downgrade is refused before DDL. Guarded fresh/prior-state/drift coverage exists; final current-revision PostgreSQL 16 and published-CI evidence is pending. |
| REQ-STORE-003 | Create all 25 required `aqa_` tables with explicit keys, relationships, checks, indexes, UTC timestamps, immutable append rows, versioned state, and specified uniqueness for bars, experiment, slots, signals, orders, jobs, outbox, and audit sequence. | Core §14.3 | `platform/storage/tables.py`; migrations | Metadata and PostgreSQL constraint tests | IMPLEMENTED_NOT_EXTERNALLY_VALIDATED | SQLAlchemy metadata and additive migrations define the exact inventory and contracts, including append-only bar revision lineage, separated semantic/provenance hashes, latest projection, symbol watermarks, and audit sequence. SQLite and offline DDL checks pass; final current-revision PostgreSQL 16/publication evidence is pending. |
| REQ-STORE-004 | Provision seven least-privilege PostgreSQL roles with the exact collector/scheduler/strategy/execution/control/read-only/migration boundaries and prove representative unauthorized writes fail; passwords never enter SQL or Git. | Core §14.4 | bootstrap SQL/migrations; role tests | PG16 role/grant integration matrix | IMPLEMENTED_NOT_EXTERNALLY_VALIDATED | Cluster bootstrap creates seven non-login authorization roles plus matching login principals, validates fixed attributes/memberships, changes passwords through libpq, and normalizes database/schema/table/column/sequence/routine/type/default ACLs. The migration is a trusted deployment-only owner with ordinary business DML self-revoked but ownership/grant authority retained; runtime roles receive scoped grants and audit views/policies. Final current-revision PostgreSQL 16 and published-CI evidence is pending. |
| REQ-STORE-005 | Repository interfaces exist only for real transaction boundaries, return domain objects, and atomically cover bars/watermarks, slots, signals, risk/latches, intents, broker events/fills, reconciliation/incidents, jobs/outbox, and audit chain. | Core §14.5 | `platform/storage/repositories.py` | Transaction rollback/concurrency tests | IMPLEMENTED_NOT_EXTERNALLY_VALIDATED | Concrete repositories cover every listed transaction boundary with SQLite behavior tests and guarded PostgreSQL paths; final current-revision PostgreSQL evidence is reported separately. |
| REQ-STORE-006 | Immutable Parquet datasets use explicit schema/order/compression/row groups, staging plus atomic rename, validated derived artifact paths, physical/logical/manifest hashes, full lineage metadata, promotability, and overwrite protection. | Core §14.6 | `platform/data/datasets.py`; artifact store | Byte/logical identity, correction, overwrite, and path tests | IMPLEMENTED_AND_VERIFIED | Offline fixtures verify deterministic logical identity, lineage, root confinement, no-replace publication, and correction-sensitive manifests; provider data remains uncommitted. |

## Scheduling, signals, risk, execution, and recovery

| ID | Requirement | Source | Implementation location | Verification | Status | Assumption |
| --- | --- | --- | --- | --- | --- | --- |
| REQ-SCHED-001 | Preserve the legacy once-per-session scheduler and add deterministic XNAS slots at every 15-minute close from 09:45 through 14:30, with ready=C+60s, deadline=C+120s, and forced-flat 15:43/15:44/15:45 timing. | Core §17 | `platform/scheduling/` | Known-answer full-session/DST/early-close tests | IMPLEMENTED_AND_VERIFIED | Exact fixture-session evidence contains 20 strategy slots and one forced-flat slot. |
| REQ-SCHED-002 | Slots use deterministic IDs and exact metadata/states; transactional 30-second leases prevent duplicate work, inspect materialized results before reclaim, never catch up after deadline, and persist/audit every terminal or waiting outcome. | Core §17 | scheduler models/service/repository | Concurrency, expiry, restart, and missed-deadline tests | IMPLEMENTED_AND_VERIFIED | SQLite concurrency/restart tests cover lease and terminal-state behavior; PostgreSQL behavior is additionally guarded. |
| REQ-SCHED-003 | Unsupported early-close sessions create no entry slots and fail closed; forced risk reduction is handled separately. | Core §17; ADR requirement 12 | calendar/scheduler; ADR | Early-close integration test | IMPLEMENTED_AND_VERIFIED | ADR 0012 records the fail-closed policy and calendar tests enforce it. |
| REQ-SIGNAL-001 | `SignalEnvelope` is immutable, versioned, deterministic, hash-bound to slot/provider/experiment/data, time-bounded, contains every active symbol and availability/action/edge/target fields, and rejects unknown or inconsistent data. | Core §18 | `platform/signals/models.py` | Known-answer hash and boundary/tamper tests | IMPLEMENTED_AND_VERIFIED | Known-answer, tamper, expiry, availability, symbol, and persistence tests cover the strict envelope. |
| REQ-SIGNAL-002 | Local provider discovery uses only packaging entry point `autonomous_quant_agent.signal_providers`; configuration selects a registered ID, and raw modules/classes/URLs/commands/source are rejected. | Core §2.2 | `platform/signals/providers.py`; package metadata | Discovery and malicious-config tests | IMPLEMENTED_AND_VERIFIED | Entry-point discovery is bounded; installed provider packages remain operator-trusted code. |
| REQ-SIGNAL-003 | Provide only `AlwaysFlatSignalProvider` and deterministic non-promotable `OfflineFixtureSignalProvider`; fixture use is offline/fake/submission-disabled and emits the exact first-slot NVDA/AMD proposal then flat. | Core §18 | provider implementations | Known-answer provider tests | IMPLEMENTED_AND_VERIFIED | Provider tests and the offline demo verify deterministic first-slot and later-flat behavior. |
| REQ-SIGNAL-004 | Paper authorization defaults to `approved=false`, reason `model_approval_not_implemented`; fixture output can never qualify for paper. | Core §18 | authorization verifier | Negative authorization tests | IMPLEMENTED_AND_VERIFIED | Default authorization and fixture-source checks deny paper eligibility. |
| REQ-RISK-001 | Preserve legacy long-only risk and add a separate signed policy receiving complete signal, position/order/account/price/security/reconciliation/equity-history/covariance/experiment/latch state; missing, stale, malformed, nonfinite, or mismatched input creates no exposure. | Core §§19,42 | `platform/risk/` | Input matrix and legacy regressions | IMPLEMENTED_AND_VERIFIED | Freshness is 30s account, 300s security, 60s reconciliation, and 120s price. |
| REQ-RISK-002 | Statistics use 20 complete sessions × 25 within-session returns (500/symbol), `ddof=1`, annualization 6552, alphabetical order, finite inputs, eigenvalue floor `1e-8`, symmetric reconstruction, and hashed inputs/outputs. | Core §19.2 | `risk/statistics.py` | Independent numeric known-answer tests | IMPLEMENTED_AND_VERIFIED | NumPy floats remain internal and canonical Decimal output is finite/hash-bound. |
| REQ-RISK-003 | Signed raw scores and volatility scaling follow the exact direction/25-bps saturation/max(sigma,0.20)/10%-vol formulas and fail flat when scores or forecast volatility cannot support a finite target. | Core §19.3 | `risk/policy.py` | Independent numeric and degenerate-case tests | IMPLEMENTED_AND_VERIFIED | Independent numeric tests cover signed and degenerate inputs. |
| REQ-RISK-004 | Apply shrink-only constraints in exact order: ±0.15 symbol, 1.00 gross, ±0.30 net, 0.60 configured groups, 0.40 correlation components above 0.80; recompute for at most eight passes to `1e-12`, else all-zero `risk_constraints_not_converged`; never redistribute clips. | Core §19.4 | `risk/policy.py` | Per-constraint/order/convergence/property tests | IMPLEMENTED_AND_VERIFIED | Direct tests cover every constraint, order, convergence, and fail-flat result. |
| REQ-RISK-005 | Hard gates block new exposure for identity, expiry/deadline, basket/gap/correction/session, freshness, latch, ambiguity, symbol/asset/shortability/listing/capability/open-order, and numeric failures; validated risk reduction toward zero remains possible. | Core §19.5 | risk eligibility | Negative matrix and compromised-proposal tests | IMPLEMENTED_AND_VERIFIED | The negative matrix proves default deny while allowing separately validated reductions. |
| REQ-RISK-006 | Rebalance threshold is 0.25% of equity; below-band positions remain only absent a required reduction, while explicit zero bypasses the band. | Core §19.6 | risk policy | Boundary tests | IMPLEMENTED_AND_VERIFIED | Exact threshold boundary and zero-target behavior are tested. |
| REQ-RISK-007 | Session loss at -2% and deployment drawdown at -15% append durable ENGAGED events, force zero and survive restart; authenticated clear requires exact acknowledgement and appends CLEARED without modifying history. | Core §19.7 | `risk/latches.py`; API control | Threshold, restart, auth, acknowledgement, append-only tests | IMPLEMENTED_AND_VERIFIED | Latch repository and operator-control tests cover restart and append-only clear. |
| REQ-RISK-008 | Persist immutable risk decisions with proposal/final targets, before/after exposures, ordered controls/reasons, source times, latch/correlation identity, and policy/signal hashes. | Core §19.8 | risk models/repository | Persistence/hash/replay tests | IMPLEMENTED_AND_VERIFIED | Signed repository reload verifies authoritative signal, latch state, hashes, and immutable receipt. |
| REQ-EXEC-001 | Preserve legacy planner and implement signed quantity targets: long/fractional quantum `0.000001` ROUND_DOWN, shorts whole-share absolute quantum `1` ROUND_DOWN, zero exact. | Core §§20.1,42 | `platform/execution/planner.py` | Positive/negative/zero rounding tests | IMPLEMENTED_AND_VERIFIED | Legacy and signed planners remain separate. |
| REQ-EXEC-002 | Support and label all ten long/short/flat increase/reduce/close/forced-flat effects. | Core §20.2 | execution models/planner | Transition state table tests | IMPLEMENTED_AND_VERIFIED | Effects are enumerated below and directly tested. |
| REQ-EXEC-003 | A sign reversal persists and completes/reconciles the closing leg, proves zero within `0.000001`, refreshes/reruns risk before the deadline, then persists/submits the opening leg; any uncertainty blocks the second leg. | Core §20.3 | execution service/reconciliation | Every reversal failure-boundary and restart test | IMPLEMENTED_AND_VERIFIED | The phase barrier and fail-closed paths are enforced by planning/service tests and ADR 0006. |
| REQ-EXEC-004 | Persist intent before side effect; deterministic client IDs are ≤48 chars and stage-unique; min notional is $25 except flatten, max intent is 15% equity, max 16 intents, reductions sort first then symbol/sequence; short eligibility is rechecked and proceeds do not fund gross. | Core §20.4 | planner/service/repository | Known-ID, sorting, bound, restart, and side-effect-order tests | IMPLEMENTED_AND_VERIFIED | Signed planner/repository/service tests cover every exact bound and intent-first ordering. |
| REQ-EXEC-005 | Enforce the exact order state machine; post-submission exceptions become `SUBMISSION_UNKNOWN`, block exposure, never auto-retry, and resolve by deterministic client ID plus reconciliation; duplicate cumulative fills/updates are idempotent. | Core §20.5 | execution models/service | State-machine, timeout, duplicate, reconnect tests | IMPLEMENTED_AND_VERIFIED | The fake-broker demo exercises acceptance ambiguity, restart lookup, and replay without resubmission. |
| REQ-EXEC-006 | Paper adapter is reachable only after all ten gates; constructs literal paper mode with no configurable trading host, contains no real-money path, and is never contacted during this program. | Core §20.6 | `execution/authorization.py`, broker adapter boundary | Gate matrix, static endpoint scan, socket denial | IMPLEMENTED_NOT_EXTERNALLY_VALIDATED | Every gate and absence of real-money/configurable-host code is tested; no paper connection or order occurred. |
| REQ-EXEC-007 | Deterministic fake broker starts with $100,000 cash/equity/buying power, restricts short proceeds, conserves signed accounting, persists restart snapshots, and models the 15 specified fill/reject/timeout/duplicate/disconnect/staleness scenarios. | Core §21 | `execution/broker.py` | Accounting/state/restart scenario matrix | IMPLEMENTED_AND_VERIFIED | Scenario/accounting/restart tests and the offline demo exercise deterministic behavior. |
| REQ-EXEC-008 | Reconciliation reconstructs signed positions from fills, applies quantity `0.000001`, cash/equity `0.01` tolerances, validates traceable shorts, classifies all specified blocking/critical discrepancies, and engages a latch on either class. | Core §22 | `execution/reconciliation.py` | Every-discrepancy and tolerance tests | IMPLEMENTED_AND_VERIFIED | Direct discrepancy/tolerance tests and persistence bundles cover clean and blocking outcomes. |
| REQ-EXEC-009 | Forced flatten claims the slot, disables entries, cancels conflicting openings, reconciles, persists exact close intents, processes fills, and proves flat by 15:45; inability creates a blocking incident rather than success. | Core §22 | execution/scheduler services | Success, partial failure, restart, and deadline tests | IMPLEMENTED_AND_VERIFIED | Success, timeout/failure, cancellation, restart, incident, and exact-flat tests are present. |

## Jobs, API, artifacts, dashboard, audit, and observability

| ID | Requirement | Source | Implementation location | Verification | Status | Assumption |
| --- | --- | --- | --- | --- | --- | --- |
| REQ-JOB-001 | Durable jobs use the seven exact states, schema payload/hash/idempotency/correlation, max 3 attempts, 60-second lease, retry delays 5/10/20, safe errors, and atomic outbox creation; PG claims use `SKIP LOCKED` with deterministic SQLite tests. | Core §23 | `platform/jobs/`; storage | Concurrency, retry, expiry, rollback, replay tests | IMPLEMENTED_NOT_EXTERNALLY_VALIDATED | SQLite state/concurrency/restart tests pass and PostgreSQL claim SQL/integration tests exist; current external PG evidence is reported separately. |
| REQ-JOB-002 | Public jobs are limited to data-quality audit, gap repair, dataset freeze, and offline demo; payloads route work but do not authorize it and never carry command/path/URL/module/code. | Core §23 | job schemas/service; API | Strict-schema and route inventory tests | IMPLEMENTED_AND_VERIFIED | Strict job schemas and the API route inventory reject arbitrary authority-bearing input. |
| REQ-API-001 | Private FastAPI binds loopback by default, authenticates a ≥32-byte bearer token with constant-time comparison, has no cookies/CORS/docs by default, caps requests at 65,536 bytes and pages at 50/100, and rate-limits reads 120/min and mutations 10/min per token. | Core §24 | `platform/control/` | Startup/auth/limit/rate/header tests | IMPLEMENTED_AND_VERIFIED | In-process limiting is intentionally for one self-hosted operator, not public distributed denial-of-service defense. |
| REQ-API-002 | Expose only the two unauthenticated health routes, 16 authenticated read routes, and six authenticated bounded job/halt/resume routes specified below; no direct trade mutation exists. | Core §24.1 | control API app/models | OpenAPI/route inventory and auth tests | IMPLEMENTED_AND_VERIFIED | Metrics is counted among authenticated read endpoints; static tests reject broker/order authority. |
| REQ-API-003 | API input is strict and versioned; errors are stable, safe, correlated, and stack-free; mutations use idempotency and audit; authenticated responses add no-store and nosniff plus restrictive CSP where applicable. | Core §24 | control middleware/models | Contract, malformed/unknown/duplicate/security-header tests | IMPLEMENTED_AND_VERIFIED | API contract tests cover unknown fields, errors, idempotency, audit, and response headers. |
| REQ-SEC-006 | Artifact IDs match `^[a-z0-9][a-z0-9._-]{0,127}$`; resolved paths stay under root and reject traversal, absolute/NUL paths, symlink components, escape, and conflicting overwrite; APIs accept IDs only. | Core §25.1 | artifact store/API schemas | Traversal/symlink/overwrite tests | IMPLEMENTED_AND_VERIFIED | Dataset/demo publication and API models enforce bounded identifiers and confined no-replace paths. |
| REQ-SEC-007 | No public input accepts a URL; provider hosts are constants; no fetch/webhook/callback/import/metadata URL exists; loopback, metadata, IPv6-loopback, and file schemes are explicitly absent/rejected. | Core §25.2 | provider/API/job schemas | Route/schema and malicious-value tests | IMPLEMENTED_AND_VERIFIED | Static route/schema/provider tests enforce the closed fixed-host boundary. |
| REQ-SEC-008 | Runtime does not use unsafe object deserialization, untrusted `eval`/`exec`/`compile`, subprocess, or arbitrary imports; Parquet/JSON/YAML inputs enforce schema, size, path, and hash; no upload route exists. | Core §25.3 | platform runtime and architecture tests | Static scan and malformed artifact tests | IMPLEMENTED_AND_VERIFIED | Architecture and malformed-input tests cover the public runtime; guarded local operator scripts remain outside public input paths. |
| REQ-UI-001 | Streamlit is read-only through the private API, uses only its API token, has no DB/broker/data credentials or order controls, and renders the specified safe health/data/scheduler/signal/exposure/risk/order/reconciliation/incident/demo summaries. | Core §26 | `platform/dashboard/` | Client/route/import/credential tests | IMPLEMENTED_AND_VERIFIED | The dashboard client has a read-route allowlist and no storage, broker, or mutation authority. |
| REQ-OBS-001 | Central JSON logs contain only bounded safe identifiers and state/reason context; structurally redact secret-bearing keys and never log headers, env/settings dumps, URLs with credentials, raw payloads, or raw sensitive exceptions. | Core §27.1; Harness §33 | `platform/observability/logging.py` | Sentinel and structured-schema tests | IMPLEMENTED_AND_VERIFIED | Recursive structural redaction and closed-field tests cover success and error payloads. |
| REQ-OBS-002 | Every consequential experiment/data/dataset/slot/signal/risk/order/reconciliation/control/job/security transition emits append-only per-stream hash-chained audit state; `aqa audit verify` validates sequence/hash. | Core §27.2 | audit repository; CLI | Known-chain, tamper, concurrency, and command tests | PARTIALLY_IMPLEMENTED | Audit chaining/verifier and major persistence/control transitions are implemented; final trace review must confirm complete event coverage across every service. |
| REQ-OBS-003 | Prometheus metrics cover specified data, readiness, slot, risk, order, reconciliation, job, API, and redaction events with bounded labels; no symbol, free text, identifier, exception, or secret causes unbounded cardinality. | Core §27.3 | `platform/observability/metrics.py` | Metric inventory/cardinality/sentinel tests | IMPLEMENTED_AND_VERIFIED | The closed metric catalog and hostile-label tests prevent unbounded series. |
| REQ-OBS-004 | Long-running services have bounded work, explicit startup/shutdown/cancellation/failure propagation, useful liveness versus readiness, no detached tasks, and no timing-sleep tests. | Harness §§33,38–39 | platform workers/health | Cancellation, timeout, shutdown, and health tests | PARTIALLY_IMPLEMENTED | Collector and durable job worker have bounded lifecycle behavior; final container startup/shutdown evidence remains external. |
...
| REQ-CI-002 | CI has least-privilege, immutable-action, locked-install quality, PG16 integration, security, twice-run socket-denied demo, container/SBOM, and Python CodeQL gates; checkout does not retain credentials and no external provider secrets exist. | Core §29.2; Harness §29 | `.github/workflows/` | Workflow lint plus successful remote runs | PARTIALLY_IMPLEMENTED | The pinned offline-quality job covers format/lint/type/tests, a digest-pinned PostgreSQL 16 service with an asserted runtime major, migration, backtest/replay, Compose, and images; the separate security/demo/container-SBOM/CodeQL jobs remain absent. |
...
| REQ-DOC-004 | Maintain the prescribed operations, developer, strategy, demo, implementation-status, data dictionary, failure, security, and evidence docs plus 12 decision records with context, alternatives, consequences, and security impact. | Core §41 | `docs/`; `docs/adr/` | Required-file and current-state audit | IMPLEMENTED_AND_VERIFIED | All prescribed documents and ADRs 0001–0012 exist with evidence/status boundaries. |
| REQ-DOC-005 | Resume evidence contains only measured test/coverage/failure/hash/transition/migration/service/benchmark facts and never planned or profitability claims. | Core §41 | `docs/resume_evidence.md` | Evidence provenance audit | IMPLEMENTED_AND_VERIFIED | The evidence file contains only commands/results actually run and explicitly excludes market-performance claims. |
| REQ-HARNESS-001 | Provide one real canonical command surface for bootstrap, build, format/check, lint, typecheck, unit/integration/regression/full tests, security, and full check, with applicable coverage/package/run/replay/demo/benchmark commands and no no-op targets. | Harness §20 | `Makefile` or equivalent | Execute each documented target | PARTIALLY_IMPLEMENTED | Makefile provides locked bootstrap, format/lint/typecheck, coverage, guarded integration, regressions, security, package smoke, demo, benchmark and full check; final complete execution pending. |
...
| REQ-TEST-003 | Cover every listed experiment, bar, correction, transaction, aggregation, calendar, watermark, slot, lease, signal, risk, transition, reversal, fill, ambiguity, restart, reconciliation, latch, flatten, API, path, secret, role, migration, backup, demo, and legacy invariant. | Core §§31,33,38 | platform unit/component/integration/e2e tests | Requirement-to-test matrix | PARTIALLY_IMPLEMENTED | Tests now cover Phase 2 schema migration, role/ACL boundaries, bar revision/latest/symbol-watermark transactions, audit append/verification, SQLite compatibility, and PostgreSQL-specific paths in addition to earlier platform, collector, and legacy subsets. Aggregation/calendar and most later platform boundaries remain; final current PostgreSQL 16 and full-suite evidence is pending. |
...
| REQ-DEMO-001 | `aqa demo` runs the exact synthetic 1Min→canonical→revision→aggregate→gap/watermark→slot→signal→risk→intent→fake-fill→reconcile→restart/replay→flatten→audit/API/evidence vertical slice with no credential/network. | Core §33 | `platform/demo.py`; CLI | Fresh-state offline end-to-end test | IMPLEMENTED_AND_VERIFIED | Evidence is labeled `OFFLINE_FIXTURE_NOT_ALPACA_EVIDENCE`. |
| REQ-DEMO-002 | Two fresh demo runs produce identical logical event, aggregate, watermark, slot, signal, risk, plan, order, fill, reconciliation, account, audit, and manifest hashes; only temp paths/PIDs/wall-clock run metadata may differ. | Core §33 | demo evidence manifest; CI | Twice-run hash comparison | IMPLEMENTED_AND_VERIFIED | Fresh temporary states produce the same logical evidence and manifest hash. |
| REQ-DEMO-003 | Inject all nine specified persistence/decision/side-effect/flatten failures; each recovers deterministically without duplication or leaves a durable fail-closed incident. | Core §33 | demo failure injection and tests | Scenario matrix | IMPLEMENTED_AND_VERIFIED | The manifest reports all nine recovery boundaries with zero duplicate side effects. |
| REQ-PERF-001 | Benchmark deterministic normalization, ingestion, aggregation, slot claim, risk, and fake execution/reconciliation with warmups/repeats, machine-readable results, environment metadata, and no unstable CI thresholds or unsupported claims. | Core §35; Harness §32 | `scripts/benchmark_pipeline.py`; `docs/performance.md` | Repeatable benchmark smoke and result schema | IMPLEMENTED_AND_VERIFIED | Benchmark smoke and schema tests cover all six operations; values are observational, not acceptance budgets. |
...
- PostgreSQL roles (`REQ-STORE-004`): `aqa_migrate` is a trusted deployment-only schema owner;
  ordinary business DML is self-revoked, but ownership/grant authority is retained and the role is
  never assigned to runtime services;
  ...
  `aqa_readonly` has `SELECT` only on explicit safe views. Each authorization role has one matching
  login principal. Collector/scheduler/strategy/execution audit reads use actor/stream-scoped views;
  control and read-only roles receive the full audit-event and audit-status safe views, and the
  read-only verifier uses that full view. The dashboard has no database role.
...
Current implementation note: `REQ-OPS-002` remains `PARTIALLY_IMPLEMENTED` pending final
integrated evidence. Compose and source contain database/bootstrap/migrations, API/dashboard,
durable jobs, offline domain workers and a runnable gated paper worker. Operational scheduler and
AlwaysFlat strategy adapters now support configured shadow/paper profiles with current clocks,
current durable data and separate database roles. Shadow proposal and risk/plan commands are real
broker-free operations. No provider activation or external order is claimed.
...
- Collector, legacy paper and target paper adapters have deterministic contract tests. External
  validation remains separate from their final local implementation checks; complete adapters may be
