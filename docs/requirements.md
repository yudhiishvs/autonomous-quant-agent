...
| REQ-FUNC-002 | Default installation requires no broker credentials, makes no external network calls, uses deterministic fixture data and a fake broker, and has submission disabled. | Core §§2,11.3,21,33,40 | `configs/platform/offline.yaml`; platform demo | Socket-denied demo and startup tests | PARTIALLY_IMPLEMENTED | The default static profile and doctor enforce fixture data, fake broker, and disabled submission without reading secrets or constructing clients; the offline runtime/demo remains absent. |
