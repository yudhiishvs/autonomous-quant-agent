...
| REQ-SEC-001 | Platform services read secrets only through the seven `AQA_*_FILE` variables; ordinary settings use the six allowed nonsecret variables and safe defaults; generic SDK credential names are rejected. | Core §6.2 | `platform/security.py`, config, Compose | Environment inventory and startup tests | PARTIALLY_IMPLEMENTED | The loader exposes a closed exact seven-variable source enum. Runtime settings, the six nonsecret defaults, generic-name rejection, service-scoped use, and Compose mounts remain absent; current `APA_*` value-based collector variables are a predecessor. |
