...
| REQ-TEST-003 | Cover every listed experiment, bar, correction, transaction, aggregation, calendar, watermark, slot, lease, signal, risk, transition, reversal, fill, ambiguity, restart, reconciliation, latch, flatten, API, path, secret, role, migration, backup, demo, and legacy invariant. | Core §§31,33,38 | platform unit/component/integration/e2e tests | Requirement-to-test matrix | PARTIALLY_IMPLEMENTED | The current 376-test offline suite covers collector and legacy subsets plus test-environment isolation; 13 PostgreSQL tests exist, but fresh PostgreSQL 16 evidence is unavailable. |
