...
Profile composition will create the required immutable `ExperimentSpec` containing those choices
...
ordered; decimals use the canonical decimal boundary. The composed experiment will have its own
hash including its signal-provider and execution-mode identity.
...
experiment will pin that expected definition hash when profile contracts are implemented.
...
platform/configuration hash and downstream data-contract checks must bind the definition hash,
runtime adapter/source mode, and dataset identity together. Shadow/live-data inputs must match the
definition's Alpaca/IEX/raw identity; offline fixture data can satisfy only an offline profile and
can never count as external-provider validation.
...
The platform-profile loader must carry an expected definition hash, then compose the full
`ExperimentSpec` with signal-provider and execution-mode identity. Runtime adapter, broker, and
submission authority remain in `PlatformConfig`. Existing legacy and collector configuration
remain unchanged until narrow compatibility adapters are deliberately introduced.

Profile and data-contract implementation must enforce the external-series/runtime-source rule
above. Until those contracts exist, this ADR accepts the ownership and identity model but does not
claim that offline bars, external bars, or their readiness checks are implemented.
...
its trusted root or omit expected-hash pinning; runtime profile construction must therefore own
the root and require the pin rather than accepting either from a remote request. Runtime
composition must also reject fixture data outside the offline profile and must never relabel it as
Alpaca data; those enforcement points remain to be implemented and tested.
